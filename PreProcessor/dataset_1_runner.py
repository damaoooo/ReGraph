...
class Dataset_1Scanner(FileScanner):
    ...
    def scan(self):
        ...
        for project_folder in os.listdir(self.root_path):
            project_path = os.path.join(self.root_path, project_folder)
            if not os.path.isdir(project_path):
                continue
            for c_dot_folder in os.listdir(project_path):
                c_dot_folder_path = os.path.join(project_path, c_dot_folder)
                ...
                if c_dot_folder.endswith(".tmp"):
                    continue
                arch, opt_level, file_name = self.info_extract(c_dot_folder)
