...
def lift_reoptimize(file_path: str, max_memory: int = 5 * unit.GB.value, timeout: int = 3600):
    ...
    try:
        m = subprocess.run(["retdec-decompiler", file_name, "-s", "-k", "--backend-keep-library-funcs", "--max-memory", str(max_memory)], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout)
        ...
        m = subprocess.run(["retdec-decompiler", "{}.re".format(file_name), "-s", "-k", "--backend-keep-library-funcs", "--max-memory", str(max_memory)], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout)
...
def runner(file_path: str, is_dataset2: bool = False):
    ...
    iter_list = file_iterator_dataset_1(file_path)
    ...
    for file in iter_list:
        ...
        p = pool.apply_async(lift_reoptimize, (file, lift_mem, ), callback=lambda _: bar1.update(1))
...
if __name__ == "__main__":
    file_path = "/home/username/Downloads/Dataset-1/"
    
    clean_up(file_path, is_dataset2=False)
    runner(file_path)